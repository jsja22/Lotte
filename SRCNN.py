...
start1 = 750
end1 = 1000
x = np.load('C:/data/LPD_competition/npy/LPD_x_128.npy',allow_pickle=True)
y = np.load('C:/data/LPD_competition/npy/LPD_x.npy',allow_pickle=True)
...
layer10 = Conv2DTranspose(filters=512,kernel_size=2,strides=2,padding='same',kernel_initializer=initializer,use_bias=False)(layer9)
...
cp = ModelCheckpoint('C:/data/LPD_competition/modelcheckpoint/srcnn.h5',monitor='val_acc',save_best_only=True, verbose=1)
...
Generator.load_weights('C:/data/LPD_competition/modelcheckpoint/srcnn.h5')
...
for j in range(start1,end1) :
    os.mkdir(f'C:/data/LPD_competition/train_SR/{j}')
    for i in range(24) :    # 폴더 안 사진의 갯수
        b = 48+i
        c = 24*(j-start1) + i
        predict = BGR2RGB(output[c])
        cv2.imwrite(f'C:/data/LPD_competition/train_SR/{j}/{b}.jpg', predict)
